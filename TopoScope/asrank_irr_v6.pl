...
# exception for VPs provided in exclvps_string by the user
my %exclvps;
if(defined($exclvps_string))
{
	# $_ represents elements in a list
...
# CUSTCONE
# compute custcone for a=>b path
# first set a=>b cc
# then add b's cone to a
# then recursively compute a's provider
sub custcone($$);
sub custcone($$)
{
	# all p2c reachable node
    my ($a, $b) = @_;
...
# CLIQUE
# whether the ASN a in the referred clique
sub clique($)
{
    my ($as) = @_;
...
# P2C_OK
# return 0 if x in y's cone or y is in the clique
# return 1 if x y is p2p
sub p2c_ok($$)
{
    my ($x, $y) = @_;
...
# C2P
# check if x=>y link is taged as c2p
sub c2p($$)
{
    my ($x, $y) = @_;
...
# P2P
# check if x=>y link is taged as p2p
sub p2p($$)
{
    my ($x, $y) = @_;
...
# P2C
# check if x=>y link is taged as p2c
sub p2c($$)
{
    my ($x, $y) = @_;
    return 0 if(!defined($r{$x}{$y}) || $r{$x}{$y} != -1);
...
# TRANSIT_DEGREE
# return link number have transfacation around ASN
sub transit_degree($)
{
    my ($as) = @_;
...
# PROVIDER_DEGREE
# return link number of the ASN to its provider
sub provider_degree($)
{
	# degree of ASN to probider
    my ($as) = @_;
...
# PEER_DEGREE
# return link number of the ASN to its peer
sub peer_degree($)
{
    my ($as) = @_;
...
# a2b cross x if a < b return -1 a > b return 1
sub peerrank($$$)
{
    my ($a, $b, $x) = @_;
    my ($i, $j);
...
# TOP_DOWN
# cmp function of sort 
# for top_down a,b
# return -1,0,1 in respect of a<b, a=b, a>b
sub top_down($$)
{
    my ($a, $b) = @_;
...
# PARSE_PATH
# transfer represents of paths from string to list
# while ruleing out exception path or ASN
sub parse_path($)
{
    my ($path) = @_;
    if($path =~ /^#/ || $path =~ /\|\|/)
	...
	# return simple parse result if filtered_flag is set
    my @path = split(/\|/, $path);
...
sub read_paths
{
	...
	foreach my $i (1 .. $#path)
	{
		...
		$data{$b}{links}{$x}++;
		$data{$x}{links}{$b}++;
		# trans: a=>x=>b transfication between x,a x,b
		$data{$x}{trans}{$a}++;
		$data{$x}{trans}{$b}++;
		# trip: a=>x=>b triplet on path
		$data{$a}{trips}{$x}{$b}++;
		$data{$b}{trips}{$x}{$a}++;
		# upstr: a>x>b backtrace
		$data{$b}{upstr}{$x}{$a}++;
		# povup: a>x>b for x as centre point
		$data{$x}{povup}{$a}{$b}++;
		...
		{	
			# trip_z: a-x-b(end) triplet on path of b is the end of path
		    $data{$a}{trip_z}{$x}{$b}++;
		}
	    }
	}
...
{
	# for clique link hashtable and AS y,x 
	# 
    my ($h, $y, $x) = @_;
...
sub python_clique
{
    ...
    print PY_OUT "import sys\n";
    print PY_OUT "sys.path.append('/home/lwd/AT/TopoScope')\n";
...
sub infer_clique($)
{
	...
	# such x cannot connext to the clique
	# for any AS lower then x, it doesn't fit the rule as well
...
$clique_string = "174 3356 3549 6939 24482 35280 39533";
