...
sub parse_path($)
{
	...
	if(clique($as))
	{
	    return () if(($clique == 1 && !clique($path[$i-1])) || $clique == 2);
	    $clique += 1;
	}
    }
...
if(defined($clique_string))
{
    ...
    if(-r $clique_string)
    {
		open(CLI, $clique_string) or die "could not open $clique_string";
		while(<CLI>)
		{
			chomp;
			$clique{$_} = 1 if(/^(\d+)/);
    ...
    elsif($clique_string =~ /^[\d\s]+$/)
    {
		$clique{$_} = 1 foreach(split(/\s+/, $clique_string));
    }
    else
    {
		print STDERR "invalid clique parameter\n";
		exit -1;
    }
	my $clique_num = scalar(keys %clique);
